...
import { Search, X, ChevronRight, Sparkles } from './Icons';
...
export const SearchView: React.FC<SearchViewProps> = ({ t, events, businesses, onNavigate }) => {
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-10 relative z-10">
        <div className="max-w-4xl w-full text-center space-y-12">
          ...
          <div className="relative group max-w-3xl mx-auto w-full animate-in zoom-in-95 duration-700 delay-500 pt-8">
            ...
            <div className="relative flex items-center bg-white/10 backdrop-blur-3xl border border-white/30 rounded-[40px] px-10 py-5 shadow-2xl">
              <div className="text-white/60 group-hover:text-blue-200 transition-colors">
              ...
              <input 
                ...
                className="flex-1 bg-transparent border-none text-white text-2xl sm:text-3xl font-black placeholder-white/30 focus:ring-0 px-8 py-4"
              />
              {query && (
                <button 
                  onClick={() => setQuery('')}
          ...
          <div className={`transition-all duration-700 overflow-hidden ${query ? 'max-h-[600px] opacity-100 mt-16 pb-20' : 'max-h-0 opacity-0'}`}>
            <div className="max-w-2xl mx-auto text-left space-y-8">
              ...
              {results.business.length > 0 && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {results.business.slice(0, 4).map(item => (
                      <button 
                        key={item.id}
