...
// Fix: Defining TOWN_NAME which was missing but used on line 119
  ...
  return (
    <div className="flex flex-col animate-in fade-in duration-700 pb-32 overflow-x-hidden">
      ...
      <section className="relative h-[85vh] w-full overflow-hidden flex-shrink-0">
        ...
        {/* Radar Status Indicator Localized */}
        <div className="absolute top-12 left-8 z-30 flex items-center gap-3 bg-white/10 backdrop-blur-xl px-5 py-3 rounded-2xl border border-white/20">
           ...
           <span className="text-[9px] font-black text-white uppercase tracking-[0.2em]">{t.common.status}</span>
        </div>
      ...
      <div className="max-w-6xl mx-auto w-full px-6 -mt-24 relative z-20 space-y-24">
        ...
        <section className="bg-gradient-to-br from-yellow-400 via-orange-400 to-yellow-500 rounded-[50px] p-10 text-white shadow-2xl relative overflow-hidden group cursor-pointer hover:scale-[1.02] transition-all" onClick={() => onNavigate(ViewState.KIDS)}>
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
            ...
            <div className="flex-1 text-center md:text-left">
              <h3 className="text-4xl font-black tracking-tighter mb-4 uppercase">{t.menu.kids}</h3>
              <p className="text-white/90 text-lg font-black leading-tight mb-8 drop-shadow-md">
                {hp.kids_big} • {t.menu.kids}
              </p>
              <div className="inline-flex bg-white text-orange-600 px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest items-center justify-center gap-3 shadow-xl">
                {hp.play} <Play size={18} fill="currentColor" />
              </div>
            </div>
          </div>
        </section>

        {/* IA CONCIERGE CARD Localized */}
        <section className="bg-gradient-to-br from-indigo-900 via-blue-900 to-[#1e1b4b] rounded-[50px] p-10 text-white shadow-2xl relative overflow-hidden group">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
            ...
            <div className="flex-1 text-center md:text-left">
              <h3 className="text-4xl font-black tracking-tighter mb-4">{t.menu.ai}</h3>
            ...
            {SHORTS_BASE.map((short, idx) => (
              <div key={short.id} className="w-56 h-96 shrink-0 rounded-[35px] overflow-hidden relative group cursor-pointer shadow-2xl border border-white/10 transition-all hover:scale-[1.02]">
                ...
                <div className="absolute bottom-6 left-6 right-6">
                  <p className="text-white font-black text-sm leading-tight mb-1 line-clamp-2">{hp.shorts_titles[idx] || TOWN_NAME}</p>
        ...
        {/* MOMENTOS PH + AI Postcard Localized */}
        <section className="space-y-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 px-2">
            {[1, 2, 3, 4].map(i => (
              <div key={i} className={`aspect-square rounded-[30px] overflow-hidden shadow-xl border border-gray-100 relative group cursor-zoom-in ${i % 2 === 0 ? 'mt-8' : ''}`}>
                <img src={`https://images.unsplash.com/photo-${1500000000000 + i * 100000}?auto=format&fit=crop&w=600&q=80`} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000" />
              </div>
            ))}
          </div>
        </section>

        {/* FEATURED EVENTS Localized */}
        <section className="space-y-8">
          <div className="flex justify-between items-end px-4">
            <div>
              <h3 className="text-[10px] font-black text-blue-600 uppercase tracking-[0.4em] mb-2">{t.menu.events}</h3>
              <h2 className="text-4xl font-black text-gray-900 tracking-tighter leading-none">{t.menu.events}</h2>
            </div>
