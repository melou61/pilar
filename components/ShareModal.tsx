...
interface ShareModalProps {
  ...
  t: any; // Translation object for 'share'
}
...
export const ShareModal: React.FC<ShareModalProps> = ({ isOpen, onClose, data, t }) => {
  ...
  // Platforms that don't support direct web sharing links easily
  const copyPlatforms = [
    {
      name: t.apps.instagram,
      icon: Instagram,
      ...
      <div className="relative w-full max-w-md bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl p-6 animate-in slide-in-from-bottom duration-300">
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          {shareLinks.map((app) => (
            ...
            >
              <div className={`w-12 h-12 rounded-full ${app.color} flex items-center justify-center text-white shadow-lg group-hover:scale-110 transition-transform`}>
                <app.icon size={24} />
              </div>
        ...
        <div className="space-y-3 mb-6">
           ...
           <div className="flex gap-3">
              {copyPlatforms.map((app) => (
                ...
                >
                   <div className={`w-10 h-10 rounded-xl ${app.color} flex items-center justify-center text-white shadow-md group-hover:scale-105 transition-transform`}>
                     <app.icon size={20} />
                   </div>
        ...
        <div className="flex items-center gap-2 p-2 bg-gray-50 rounded-xl border border-gray-200">
          <input 
            ...
            readOnly 
            value={data.url} 
            className="flex-1 bg-transparent border-none text-sm text-gray-500 focus:ring-0 px-2"
          />
          <button 
            onClick={handleCopy}
